...
CANCEL_RE = re.compile(r'^(?:pare|para|cancele|cancelar|cancela|interrompa|interromper)(?:\s+isso)?[.! ]*$', re.I)
REPLACE_RE = re.compile(r'^(?:esquece isso|esqueça isso|cancela isso|cancele isso|deixa isso|deixe isso)(?:\s+e\s+|\s*,\s*)(.+)$', re.I | re.S)

_HARD_HINTS = (
    'analise profundamente','análise profunda','detalhadamente','passo a passo','arquitetura','refatore','refatorar',
    'investigue','diagnostique','causa raiz','root cause','revise o projeto','otimize tudo','corrija tudo',
)
_RESEARCH_HINTS = (
    'pesquise','pesquisar','procure','buscar','busque','encontre','encontrar','colete','coletar','investigue','levante',
    'mapear','audite','auditar','possível cliente','possivel cliente','lead','presença digital','presenca digital',
    'traga os dados','trazer os dados','dados de 1 empresa','precisa de um site','precisando de um site',
)
_MULTI_HINTS = (
    'todos os dados','com todos os dados','links','imagens','contatos','telefone','whatsapp','instagram','compare','comparar',
    'o melhor','melhor opção','melhor opcao','verifique','validar','fontes',
)
_IMPLEMENT_HINTS = (
    'implemente','implementar','desenvolva','desenvolver','corrija o projeto','aplique no projeto','faça tudo','faca tudo',
    'implemente tudo','implementar tudo','refatore o projeto','revise todo','revise toda',
)
_MISSION_HINTS = ('missão:','missao:','execute até finalizar','execute ate finalizar','trabalhe até concluir','trabalhe ate concluir','faça até finalizar','faca ate finalizar')
...
_EXECUTOR = ThreadPoolExecutor(max_workers=max(2, min(12, int(os.getenv('HERMES_GATEWAY_WORKERS', '6')))), thread_name_prefix='hermes-fastpath')
...
def _trace_id() -> str: return uuid.uuid4().hex[:12]
    ...
    with _PROC_LOCK:
        if proc is None:
            ...
            if key and key in _CHAT_TRACES:
                ...
                if not _CHAT_TRACES[key]: _CHAT_TRACES.pop(key, None)
        else:
            _ACTIVE_PROCS[trace] = proc
            if key:
                _CHAT_TRACES.setdefault(key, []).append(trace)
...
def _classify_complexity(text: str, is_cron: bool = False) -> str:
    ...
    if not low: return 'fast'
    if re.match(r'^\s*(?:responda|responde)\s+(?:apenas|somente)\s*:', low): return 'fast'
    if _mission_score(text) >= 6: return 'mission'
    score = 0; words = len(re.findall(r'\S+', low))
    if words >= 110 or len(low) >= 750: score += 2
    elif words >= 65 or len(low) >= 420: score += 1
    score += sum(1 for hint in _HARD_HINTS if hint in low)
    if re.search(r'\b(?:analise|compare|investigue|diagnostique|planeje|refatore)\b', low): score += 1
...
def _authorized(gateway: Any, source: Any) -> bool:
    ...
    for store in stores:
        ...
        if callable(approved):
            try:
                if bool(approved(platform, str(user_id))): return True
            except Exception: pass
    env_name = {'telegram':'TELEGRAM_ALLOWED_USERS','discord':'DISCORD_ALLOWED_USERS','whatsapp':'WHATSAPP_ALLOWED_USERS','whatsapp_cloud':'WHATSAPP_CLOUD_ALLOWED_USERS','slack':'SLACK_ALLOWED_USERS','signal':'SIGNAL_ALLOWED_USERS'}.get(platform)
    if env_name and _allowlist_contains(_env_value(env_name), user_id): return True
    if _allowlist_contains(_env_value('GATEWAY_ALLOWED_USERS'), user_id): return True
    if platform and _env_value(f'{platform.upper()}_ALLOW_ALL_USERS').lower() in {'1','true','yes'}: return True
    if _env_value('GATEWAY_ALLOW_ALL_USERS').lower() in {'1','true','yes'}: return True
    return False


def _remember_channel(source: Any) -> None:
    platform = str(getattr(getattr(source, 'platform', None), 'value', '') or '').strip().lower()
    chat_id = str(getattr(source, 'chat_id', '') or '').strip(); user_id = str(getattr(source, 'user_id', '') or '').strip()
    if platform != 'telegram' or not chat_id: return
    try:
        path = Path.home() / '.hermes' / 'core-v2' / 'state' / 'channel_state.json'
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps({'platform':platform,'chat_id':chat_id,'user_id':user_id,'updated_at':int(time.time())}, ensure_ascii=False, indent=2), encoding='utf-8')
    except Exception as exc: logger.warning('fastpath channel save failed: %s', exc)


def _run_process(args: list[str], *, cwd: str, timeout: int, source: Any, trace: str, tier: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy(); env['HERMES_TRACE_ID'] = trace; env['HERMES_COMPLEXITY'] = tier
    ...
    try:
        ...
        try:
            proc.terminate(); stdout, stderr = proc.communicate(timeout=2)
        except Exception:
...
def _run_core(work: Work) -> str:
    root = Path.home() / '.hermes' / 'core-v2'; py = root / 'venv' / 'bin' / 'python'; core = root / 'core_entry.py'
    proc = _run_process([str(py), str(core), work.text], cwd=str(root), timeout=_TIMEOUTS.get(work.tier,45), source=work.source, trace=work.trace_id, tier=work.tier)
    if proc.returncode != 0: return f"⚠️ Não consegui concluir essa resposta. {(proc.stderr or proc.stdout)[-500:].strip()}"
...
def _run_time(work: Work) -> str:
    ...
    proc = _run_process([str(py), str(manager), '--text-b64', encoded], cwd=str(root), timeout=_TIMEOUTS['cron'], source=work.source, trace=work.trace_id, tier='cron')
    raw = (proc.stdout or '').strip(); marker='HERMES_CRON_RESULT:'
    if proc.returncode != 0: return f"⚠️ Não consegui concluir essa rotina agora. {(proc.stderr or raw)[-500:].strip()}"
    return raw.rsplit(marker,1)[-1].strip() if marker in raw else (raw or 'Rotina processada.')


def _run_flow_submit(work: Work) -> str:
    root = Path.home() / '.hermes' / 'core-v2'; py = root / 'venv' / 'bin' / 'python'; submit = root / 'flow_submit.py'
    encoded = base64.urlsafe_b64encode(work.text.encode()).decode()
    proc = _run_process([str(py), str(submit), '--text-b64', encoded], cwd=str(root), timeout=_TIMEOUTS['mission'], source=work.source, trace=work.trace_id, tier='mission')
    if proc.returncode != 0: return f"⚠️ Não consegui iniciar o fluxo. {(proc.stderr or proc.stdout)[-500:].strip()}"
    return (proc.stdout or '').strip()


def _split_message(text: str, limit: int = 3600) -> list[str]:
    text = (text or '').strip()
    if not text: return ['Não encontrei uma resposta confiável para isso ainda.']
    chunks=[]
    while len(text)>limit:
        ...
        if cut<=0: cut=limit
        chunks.append(text[:cut].strip()); text=text[cut:].lstrip()
...
async def _send_all(adapter: Any, chat_id: Any, chunks: list[str], trace: str) -> None:
    for i, chunk in enumerate(chunks):
        ...
        if i+1<len(chunks): await asyncio.sleep(0.1)
...
def _schedule_send(loop: asyncio.AbstractEventLoop, gateway: Any, source: Any, text: str, trace: str) -> None:
    ...
    def submit() -> None:
        ...
        except Exception: logger.exception('fastpath send scheduling failed trace=%s', trace)
...
def _process(work: Work) -> None:
    try:
        ...
        _schedule_send(work.loop, work.gateway, work.source, f'Não consegui concluir essa solicitação agora. Detalhe: {exc}', work.trace_id)
    finally:
        _PENDING.release()


def pre_gateway_dispatch(event: Any = None, **kwargs):
    if event is None: event = kwargs.get('event')
    gateway = kwargs.get('gateway')
    if event is None or gateway is None: return None
    source = getattr(event,'source',None); text=str(getattr(event,'text','') or '').strip(); trace=_trace_id()
    if not text or source is None: return None
    if not _authorized(gateway, source): return None
    ...
    if replace:
        _cancel_latest(source,trace); text=replace.group(1).strip()
    elif CANCEL_RE.match(text):
        cancelled=_cancel_latest(source,trace)
        answer='Cancelei a resposta que estava sendo processada.' if cancelled else 'Não há resposta transitória em processamento. Para cancelar um fluxo longo, diga “cancelar missão <ID>”.'
        _schedule_send(loop,gateway,source,answer,trace)
        return {'action':'skip','reason':'explicit-cancel'}

    is_time=bool(TIME_RE.search(text)); tier=_classify_complexity(text,is_cron=is_time)
    if not _PENDING.acquire(blocking=False):
        _schedule_send(loop,gateway,source,'Tenho muitos fluxos locais em andamento agora. Tente novamente em alguns instantes.',trace)
