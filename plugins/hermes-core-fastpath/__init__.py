...
def _run_core(text: str) -> str:
    ...
    if proc.returncode != 0:
        return f"⚠️ Hermes Core retornou erro: {(proc.stderr or proc.stdout)[-600:].strip()}"
...
def _run_cron(text: str) -> str:
    ...
    if proc.returncode != 0:
        return f"⚠️ Cron Manager retornou erro: {(proc.stderr or raw)[-600:].strip()}"
...
def pre_gateway_dispatch(**kwargs):
    ...
    is_core = bool(CORE_RE.search(text))
    if not (is_cron or is_core):
        return None

    try:
        reply = _run_cron(text) if is_cron else _run_core(text)
    except subprocess.TimeoutExpired:
        reply = "⚠️ A ação local excedeu 25s e foi interrompida."
    except Exception as exc:
        reply = f"⚠️ Falha no fastpath local: {exc}"
