...
V9_SRC="$SCRIPT_DIR/runtime/hermes_fast_router_v9.py"
...
[[ -f "$V9_SRC" ]] || { echo "Missing $V9_SRC" >&2; exit 1; }
...
V8_PY="$RUNTIME_DIR/hermes_fast_router_v8.py"
ROUTER_PY="$RUNTIME_DIR/hermes_fast_router_v9.py"
...
cp -a "$V7_SRC" "$V7_PY"
cp -a "$V8_SRC" "$V8_PY"
cp -a "$V9_SRC" "$ROUTER_PY"
chmod 700 "$BASE_PY" "$V7_PY" "$V8_PY" "$ROUTER_PY"

cat > "$SERVICE" <<EOF
[Unit]
Description=Hermes Local Fast Router v9
...
for _ in $(seq 1 30); do
  ...
  if grep -q '"router_version": 9' <<<"$health"; then
    ready=1
    break
  fi
  sleep 1
...
if [[ "$ready" -ne 1 ]]; then
  echo "Fast router v9 did not become ready. Restoring config." >&2
...
echo "health=$health"
echo
echo "Cron + Core v2.1 Telegram fastpaths enabled."
echo "Examples:"
echo "  crie uma rotina todo dia às 8h para me mandar notícias de Flutter"
echo "  status da vps"
echo "  corrija tudo"
echo "  o que voce corrigiu?"
echo "  reinicie o gateway"
