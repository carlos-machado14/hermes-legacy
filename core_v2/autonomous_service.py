...
def _send(text: str) -> bool:
    ok,_=send_telegram(text,attempts=1)
    return ok
...
def _last_progress_ts() -> int:
    rows=recent_decisions(150)
    kinds={'task_completed','goal_progress','goal_completed','next_action','decision','autonomous_action_completed'}
...
def _overdue_high_tasks() -> list[dict[str, Any]]:
    ...
    for task in list_tasks(status='todo'):
        if task.get('priority')!='high' or not task.get('due'):
            continue
        try:
            due=datetime.fromisoformat(str(task.get('due')).strip()).date()
        except Exception:
            continue
        if due<today:
            out.append(task)
...
def _initiative_cycle(runtime: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if runtime.get('initiative_bucket')==bucket:
        return []
    created=discover_initiatives(limit=5)
    runtime['initiative_bucket']=bucket
    runtime['updated_at']=int(time.time())
    _write_json(RUNTIME_FILE,runtime)
    if created:
        record('initiative_discovery',f'{len(created)} ação(ões) autônoma(s) criada(s) a partir dos objetivos.')
    return created
...
def _next_action_line() -> str | None:
    try:
        state = get_agent_state(refresh_state=True)
        actions = list(state.get('next_actions') or [])
        if actions:
            return f"➡️ Próximo passo útil: {actions[0].get('title')}"
    except Exception:
        pass
    return None
...
def _autonomy_cycle(runtime: dict[str, Any]) -> bool:
    ...
    if runtime.get('autonomy_bucket')==bucket:
        return False
    report=run_cycle()
    _mark(runtime,'autonomy_bucket',bucket)
    ...
    result_text = '\n'.join(str(a.get('result') or a.get('title') or '') for a in executed[:5]).strip()
    try:
        refresh_agent_state(
            recent_result=result_text or None,
            pending_decision=approval_text or None,
        )
    except Exception:
        pass
...
def tick() -> None:
    proactive=load_settings()
    autonomy=load_autonomy_settings()
    if not proactive.get('enabled') and not autonomy.get('enabled'):
        return

    # Subconsciente operacional: mantém uma visão persistente e atualizada mesmo
    # quando o usuário não está enviando mensagens.
    try:
        refresh_agent_state()
    except Exception as exc:
        print(f'agent state refresh error: {exc}', flush=True)
    ...
    morning=int(proactive.get('morning_hour',8))%24
    evening=int(proactive.get('evening_hour',19))%24
    ...
    if stats.get('failed',0)>0 and _slot(runtime,'schedule_failure_alert',day):
        if _send(f"⚠️ Detectei {stats['failed']} falha(s) de entrega em lembretes nas últimas 24h. Estou mantendo isso como pendência operacional para acompanhamento."):
    ...
    stall_hours=max(1,int(proactive.get('stall_hours',24)))
    ...
    if goals and last_progress and time.time()-last_progress>=stall_hours*3600 and _slot(runtime,'stall_alert',day):
        if _send('🧭 Hermes — percebi que seus objetivos estão sem avanço recente.\n\n'+recommendation()):
            ...
            _mark(runtime,'stall_alert',day)


def main() -> int:
    print('Hermes Autonomous Assistant v5.2 — agent loop ativo',flush=True)
    while True:
        try:
            tick()
        except Exception as exc:
