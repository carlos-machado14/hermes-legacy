...
def _money_advice() -> str:
    ...
    if opportunities:
        out.append('')
        out.append('Melhores oportunidades agora:')
        for i, item in enumerate(opportunities[:5], 1):
            out.append(f"{i}. {item.get('title')} | score={item.get('score', 0)}/100")
    if tasks:
        high = [t for t in tasks if t.get('priority') == 'high'] or tasks
        out.append('')
        out.append('Próximas ações:')
        for i, task in enumerate(high[:5], 1):
            out.append(f"{i}. {task.get('title')}")
    out.append('')
    out.append('Minha prioridade é transformar isso em execução e receita, não só listar ideias.')
    return '\n'.join(out)
...
def _today_plan() -> str:
    ...
    if not opportunities:
        seed_from_profile(); opportunities = list_opportunities()

    out = ['Plano de hoje:']
    if goals:
        out.append(f"Objetivo principal: {goals[-1].get('title')}")
    high = [t for t in tasks if t.get('priority') == 'high'] or tasks
    if high:
        ...
        for i, task in enumerate(high[:5], 1):
            out.append(f"{i}. {task.get('title')}")
    if opportunities:
        out.append(f"Melhor oportunidade atual: {opportunities[0].get('title')} (score {opportunities[0].get('score',0)}/100)")
    if not tasks:
        out.append('Ainda não há tarefas pendentes; peça "plano do objetivo <ID>" para gerar o plano.')
    return '\n'.join(out)
...
def handle(text: str) -> str | None:
    ...
    money_phrases = (
        'oportunidade de ganhar dinheiro', 'oportunidades para ganhar dinheiro',
        'oportunidades de renda', 'oportunidade de renda', 'oportunidades de negocio',
        'oportunidades de negócio', 'oportunidade de negocio', 'oportunidade de negócio',
        'como posso ganhar dinheiro', 'como ganhar dinheiro', 'formas de ganhar dinheiro',
        'com base minhas informações', 'com base nas minhas informações',
        'com base no meu perfil', 'usando meu perfil',
    )
    if any(k in low for k in money_phrases):
        seed_from_profile()
        return _money_advice()
    if low in {'oportunidades', 'minhas oportunidades'}:
        return opportunities_summary()

    if any(k in low for k in ('o que fazemos hoje', 'o que faço hoje', 'qual a prioridade', 'qual é a prioridade', 'o que devo fazer hoje')):
        return _today_plan()
