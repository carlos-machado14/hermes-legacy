...
def handle(text: str) -> str | None:
    ...
    if any(low.startswith(k) for k in ('crie objetivo ', 'criar objetivo ', 'novo objetivo ')):
        title = _after(t, ('crie objetivo', 'criar objetivo', 'novo objetivo'))
        g = create_goal(title)
    ...
    if low.startswith('plano do objetivo ') or low.startswith('planeje objetivo '):
        ref = _after(t, ('plano do objetivo', 'planeje objetivo'))
        return plan_summary(ref)
    ...
    if any(low.startswith(k) for k in ('crie tarefa ', 'criar tarefa ', 'nova tarefa ')):
        title = _after(t, ('crie tarefa', 'criar tarefa', 'nova tarefa'))
        task = create_task(title)
        return f"Tarefa criada: [{task['id']}] {task['title']}"

    if low.startswith('concluir tarefa ') or low.startswith('marque tarefa '):
        ref = _after(t, ('concluir tarefa', 'marque tarefa'))
        ref = re.sub(r'\s+como\s+conclu[ií]da.*$', '', ref, flags=re.I).strip()
    ...
    if low.startswith('lembre que eu sei ') or low.startswith('eu sei '):
        skill = _after(t, ('lembre que eu sei', 'eu sei'))
        add_skill(skill)
        return f'Habilidade registrada: {skill}'

    if low.startswith('lembre que '):
        fact = _after(t, ('lembre que',))
        remember_fact('nota_' + str(abs(hash(fact)))[:8], fact)
    ...
    if low.startswith('pesquise ') or low.startswith('pesquisar '):
        query = _after(t, ('pesquise', 'pesquisar'))
