...
def _run_core(message: str) -> tuple[int,str]:
    p=subprocess.run([str(ROOT/'venv/bin/python'),str(ROOT/'hermes_core.py'),message],text=True,capture_output=True,timeout=45,cwd=str(ROOT))
    return p.returncode,(p.stdout or p.stderr or '').strip()


class Handler(BaseHTTPRequestHandler):
    server_version='HermesCoreAPI/3.5'
    ...
    def do_GET(self) -> None:
        ...
        if path=='/health': _json(self,200,{'ok':True,'version':'3.5','api':'active','mode':'business-opportunity-os'})
...
def main() -> int:
    ...
    print(f'Hermes Core API 3.5 listening on http://{HOST}:{PORT}',flush=True); server.serve_forever(); return 0
