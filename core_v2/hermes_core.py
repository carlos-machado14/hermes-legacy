...
def llm(prompt: str, system: str | None = None, max_tokens: int | None = None) -> str:
    system = system or ('Voce e Hermes Core v3.5, um agente pessoal e de negocios local-first. '
                        'Use contexto persistente, objetivos, tarefas, leads, CRM e aprendizados quando disponiveis. '
...
def system_health() -> dict:
    vm=psutil.virtual_memory(); swap=psutil.swap_memory(); disk=psutil.disk_usage('/')
    return {'cpu_percent':psutil.cpu_percent(interval=0.25),'ram_percent':vm.percent,
            'ram_available_mb':round(vm.available/1024/1024),'swap_percent':swap.percent,
            'disk_percent':disk.percent,'load':[round(v,2) for v in os.getloadavg()],
...
def local_status() -> dict:
    data=system_health(); data['services']={
    ...
    try:
        with httpx.Client(timeout=3) as client:
            r=client.get(f'{BASE_URL}/models'); data['llm_http']=r.status_code; data['llm_ready']=r.status_code==200
    except Exception as e:
        data['llm_http']=None; data['llm_ready']=False; data['llm_error']=str(e)
    return data
...
def route(text: str) -> str:
    t=text.lower().strip()
    if any(k in t for k in ('status da vps','saude da vps','saúde da vps','verifique a vps','como esta a vps','como está a vps')) or t in {'status','health','/health'}: return 'health'
    if any(k in t for k in ('servicos do hermes','serviços do hermes','status dos servicos','status dos serviços')): return 'services'
    if any(k in t for k in ('containers','docker ps','status do docker')): return 'docker'
    if any(k in t for k in ('status das crons','status da cron','cron status')): return 'cron_status'
    if any(k in t for k in ('listar crons','lista de crons','cron list')): return 'cron_list'
    if any(k in t for k in ('quais projetos','listar projetos','liste os projetos','meus projetos')): return 'projects'
    if any(k in t for k in ('incidentes','incidentes recentes','problemas recentes','falhas recentes')): return 'incidents'
    if 'logs do gateway' in t or 'log do gateway' in t: return 'gateway_logs'
    if 'logs do llm' in t or 'log do llm' in t or 'logs do modelo' in t: return 'llm_logs'
    if 'logs do router' in t or 'log do router' in t: return 'router_logs'
    return 'llm'


def _report_text(report: dict) -> str: return json.dumps(report,ensure_ascii=False).lower()
...
def deterministic_diagnosis(plan_name: str, report: dict) -> str | None:
    ...
    if plan_name=='diagnose_crons':
        ...
        if 'failed' in text and 'cron' in text: return 'Diagnostico rapido:\n- Existem crons com falha registrada.\n- Consulte ultima execucao e logs.'
        if 'gateway is running' in text and 'active job' in text: return 'Diagnostico rapido:\n- Scheduler e gateway estao ativos.\n- Nenhum padrao conhecido de erro nas evidencias.'
    if plan_name=='diagnose_services':
        if 'inactive' in text or 'failed' in text: return 'Diagnostico rapido: ha pelo menos um servico inativo ou com falha.'
        if 'active' in text: return 'Diagnostico rapido: os servicos principais estao ativos.'
    return None


def diagnose_with_plan(text: str, plan_name: str) -> str:
    report=execute_plan(plan_name); fast=deterministic_diagnosis(plan_name,report)
    if fast: return fast
    evidence=compact_for_llm(report,max_chars=2200)
    prompt=('Analise somente as evidencias reais abaixo. Responda em portugues em no maximo 5 linhas: causa provavel, evidencia principal e proxima acao segura. Nao invente dados.\n\n'
            f'Pedido: {text}\nEvidencias:\n{evidence}')
    try: return llm(prompt,max_tokens=120)
    except Exception: return 'Diagnostico coletado, mas o LLM nao respondeu. Evidencias:\n'+pretty(report)


def _format_history(rows: list[dict]) -> str:
    if not rows: return 'Nenhuma acao de recuperacao registrada ainda.'
    out=['Historico operacional recente:']
    for item in rows[-10:]:
        stamp=time.strftime('%d/%m %H:%M',time.localtime(int(item.get('ts',0)))); status='OK' if item.get('ok') is True else ('FALHA' if item.get('ok') is False else '-')
...
def _format_projects() -> str:
    ...
    if not projects: return 'Nenhum projeto cadastrado ainda.'
    out=['Projetos cadastrados:']
    for p in projects:
        parts=[str(p.get('name'))]
        if p.get('health_url'): parts.append(f"health={p['health_url']}")
        if p.get('services'): parts.append(f"servicos={len(p['services'])}")
...
def _format_incidents() -> str:
    ...
    if not rows: return 'Nenhum incidente registrado.'
...
def _safe_direct_action(text: str) -> str | None:
    ...
    if t in {'ferramentas','tools','listar ferramentas'} or 'quais ferramentas' in t: return pretty(list_tools())
    if t.startswith('reinicie ') or t.startswith('reiniciar '):
        for alias,service in SERVICE_ALIASES.items():
            if alias in t: return pretty(call_tool('system.restart_service',{'service':service}))
    return None


def ask(text: str) -> str:
    business_reply=handle_business_agent(text)
    if business_reply is not None: return business_reply
    personal_reply=handle_personal_agent(text)
    if personal_reply is not None: return personal_reply
    project_reply=handle_project_command(text)
    if project_reply is not None: return project_reply
    direct=_safe_direct_action(text)
    if direct is not None: return direct
    plan=classify_plan(text)
    if plan: return diagnose_with_plan(text,plan)
    r=route(text)
    if r=='health': return format_health(local_status())
    if r=='projects': return _format_projects()
    if r=='incidents': return _format_incidents()
    if r in {'services','docker','cron_status','cron_list','gateway_logs','llm_logs','router_logs'}: return pretty(run_tool(r))
    return llm(text)
...
def main() -> int:
    ...
    print('Hermes Core v3.5 - Business & Opportunity OS',flush=True)
    while True:
        try: text=input('\nVoce > ').strip()
        except (EOFError,KeyboardInterrupt): print(); return 0
        if text.lower() in {'sair','exit','quit'}: return 0
        if text:
