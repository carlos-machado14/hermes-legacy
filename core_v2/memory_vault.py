...
def write_note(section: str, title: str, body: str, *, filename: str | None = None) -> Path:
    _ensure()
    section = section if section in SECTIONS else 'conversations'
...
def append_daily(role: str, text: str) -> Path:
    _ensure()
    now = datetime.now()
    path = VAULT / 'daily' / f'{now:%Y-%m-%d}.md'
    if not path.exists():
        path.write_text(f'# Diário {now:%Y-%m-%d}\n\n', encoding='utf-8')
    who = 'Usuário' if role == 'user' else 'Hermes'
    with path.open('a', encoding='utf-8') as f:
        f.write(f'## {now:%H:%M:%S} — {who}\n\n{text.strip()}\n\n')
    index_file(path)
    return path
...
def index_file(path: Path) -> None:
    ...
    with _db() as conn:
        conn.execute(
            'INSERT INTO documents(path,title,content,mtime,indexed_at) VALUES(?,?,?,?,?) '
...
def refresh_index() -> int:
    ...
    with _db() as conn:
        for path, mtime in conn.execute('SELECT path,mtime FROM documents'):
            known[str(path)] = float(mtime)
    count = 0
    present: set[str] = set()
    for path in VAULT.rglob('*.md'):
        ...
        if abs(known.get(rel, -1) - mtime) > 0.001:
            index_file(path); count += 1
    stale = set(known) - present
    if stale:
        with _db() as conn:
            conn.executemany('DELETE FROM documents WHERE path=?', [(p,) for p in stale])
    return count


def _terms(query: str) -> list[str]:
    stop = {'para','com','que','uma','uns','umas','dos','das','de','do','da','em','no','na','nos','nas','e','o','a','os','as','meu','minha','meus','minhas','isso','essa','esse'}
    words = re.findall(r'[\wÀ-ÿ]{3,}', query.lower())
...
def retrieve(query: str, *, limit: int = 5, max_chars: int = 3600) -> str:
    ...
    if not terms:
        return ''
    with _db() as conn:
        rows = conn.execute('SELECT path,title,content,indexed_at FROM documents').fetchall()
    ...
    for path, title, content, indexed_at in rows:
        ...
        for term in terms:
            ...
            if n:
                score += min(n, 8) * 2.0
                if term in str(title).lower(): score += 3.0
        if score <= 0:
            continue
...
def sync_state_snapshots() -> None:
    ...
    for source, section, title in mappings:
        ...
        try:
            ...
            write_note(section, title, body, filename=f'{_slug(title)}.md')
        except Exception:
            continue
